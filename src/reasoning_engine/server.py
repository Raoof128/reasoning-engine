...
def _coerce_json_array(value: object, field_name: str) -> list:
    """Accept native JSON arrays or JSON-encoded array strings from MCP clients."""
...
def register_branch(
    ...
    trace_data = validate_string_list(
        _coerce_json_array(trace, "trace"),
        "trace",
        max_items=MAX_TRACE_STEPS,
        max_item_chars=MAX_TRACE_STEP_CHARS,
    )
    source_data = validate_sources(_coerce_json_array(sources, "sources"))
...
def save_to_memory(
    session_id: str,
    query: str,
    key_learnings: list[str] | str,
    domain_tags: list[str] | str,
) -> str:
    ...
    learnings = validate_string_list(
        _coerce_json_array(key_learnings, "key_learnings"),
        "key_learnings",
        max_items=MAX_LEARNINGS,
        max_item_chars=MAX_LEARNING_CHARS,
    )
    tags = validate_string_list(
        _coerce_json_array(domain_tags, "domain_tags"),
        "domain_tags",
        max_items=MAX_TAGS,
...
def evidence_gap_questions_tool(query: str, claims: list[str] | str) -> str:
    """Generate verification questions for claims before final synthesis."""
    query = validate_text(query, "query", MAX_QUERY_CHARS)
    claim_list = validate_string_list(
        _coerce_json_array(claims, "claims"),
        "claims",
        max_items=MAX_TRACE_STEPS,
        max_item_chars=MAX_TRACE_STEP_CHARS,
    )
...
def get_scholar_auth_status(dummy: object | None = None) -> str:
