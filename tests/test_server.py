...
async def test_init_research_session(client):
    async with client:
        result = await client.call_tool("init_research_session", {"query": "How do process reward models work?"})
...
async def test_full_workflow(client):
    async with client:
        # Init
        init_result = await client.call_tool("init_research_session", {"query": "Simple test query"})
        session = json.loads(_text(init_result))
        sid = session["session_id"]

        # Register branch
        branch_result = await client.call_tool("register_branch", {
            "session_id": sid,
        ...
        # Score branch
        await client.call_tool("score_branch", {
            "session_id": sid, "branch_id": branch["branch_id"],
            "q_score": 0.9, "advantage": 0.4, "critique": "Strong analysis", "confidence": 0.85,
        })
...
async def test_sanitize_content(client):
    async with client:
        result = await client.call_tool("sanitize_content", {"raw_text": "Hello <script>bad</script> world"})
...
async def test_select_next_branches(client):
    async with client:
        init = await client.call_tool("init_research_session", {"query": "Compare beam search and MCTS for reasoning tasks"})
        session = json.loads(_text(init))
        sid = session["session_id"]

        for trace in [["Path A"], ["Path B"], ["Path C"]]:
            br = await client.call_tool("register_branch", {"session_id": sid, "trace": json.dumps(trace)})
            branch = json.loads(_text(br))
            await client.call_tool("score_branch", {
                "session_id": sid, "branch_id": branch["branch_id"],
                "q_score": 0.5, "advantage": 0.2, "critique": "OK", "confidence": 0.6,
            })
